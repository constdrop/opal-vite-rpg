...
# 敵の反撃（敵のターン）
def enemy_turn
  return if @enemy.hp <= 0 # 敵が倒れていたら反撃しない
  ...
  `#{@log_area}.innerHTML += "<b>#{@enemy.name}のはんげき！#{damage}のダメージ！</b><br>"`
  
  if @player.hp <= 0
    `#{@log_area}.innerHTML += "<h2 style='color:red;'>勇者は たおれた... GAME OVER</h2>"`
...
`document.getElementById('attack-btn').addEventListener('click', function() {`
  return if !@player || @player.hp <= 0 || @enemy.hp <= 0
...
4. **決着**: どちらかのHPが0になるまでボタンを押して戦います。

## チャレンジ

- **かいふくの実装**: `heal-btn` を押したときに、自分のHPを `15` 回復させる処理を書いてみよう（ただし最大HPを超えないように注意！）。
